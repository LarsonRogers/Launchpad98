...
# Reuse step sequencer base class and constants to keep OSD mode flags aligned.
from .StepSequencerComponent import StepSequencerComponent, ButtonElement, \
    QUANTIZATION_NAMES, STEPSEQ_MODE_SCALE_EDIT
...
class StepSequencerComponent2(StepSequencerComponent):
    ...
    def _update_OSD(self):
        if self._osd != None:
            # Publish melodic step sequencer mode/attributes to the OSD.
            if self._mode == STEPSEQ_MODE_SCALE_EDIT:
                self._osd.set_mode('Melodic Step Sequencer - Scale')
                self._osd.mode_id = "melodic_stepseq_scale"
            else:
