...
class InstrumentControllerComponent(CompoundComponent):
    ...
    def setup_quick_scale_mode(self):
        ...
        # Ensure the OSD reflects quick scale sub-mode changes immediately
        self._update_OSD()
    ...
    def _update_OSD(self):
        if self._osd != None:
            if self._scales.is_enabled():
                self._osd.mode = "Instrument - Scale"
                self._osd.mode_id = "instrument_scale"
            elif self._scales.is_quick_scale:
                self._osd.mode = "Instrument (quick scale)"
                if self._quick_scale_root == 0:
                    self._osd.mode_id = "quick_scale_root"
                elif self._quick_scale_root == 1:
                    self._osd.mode_id = "quick_scale_mode"
                else:
                    self._osd.mode_id = "quick_scale_repeat"
            else:
