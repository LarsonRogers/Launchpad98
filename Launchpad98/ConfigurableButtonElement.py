...
class ConfigurableButtonElement(ButtonElement):
    ...
    def send_value(self, value, **k):
        if self._control_surface is not None and hasattr(self._control_surface, "_cache_led_value"):
            cache_value = value
            if value is ON_VALUE:
                cache_value = self._on_value
            elif value is OFF_VALUE:
                cache_value = self._off_value
            if type(cache_value) is int or isinstance(cache_value, str):
                self._control_surface._cache_led_value(self, cache_value)
        if value is ON_VALUE:
    ...
    def _draw_skin(self, value):
        if self._control_surface is not None and hasattr(self._control_surface, "_cache_led_value"):
            self._control_surface._cache_led_value(self, value)
