...
class MainSelectorComponent(ModeSelectorComponent):
    ...
    def __init__(self, matrix, top_buttons, side_buttons, config_button, osd, control_surface, note_repeat, c_instance):
        ...
        # One-shot guard to avoid spamming OSD updates when enabling melodic step seq.
        self._stepseq2_active = False
    ...
    def _setup_sub_mode(self, mode):
        ...
        if mode == "instrument":
            ...
            self._update_control_channels()
            self._mode_index = 7
            # Ensure OSD switches immediately when cycling User 2 sub-modes.
            self._osd.clear()
            self._osd.mode = "Melodic Step Sequencer"
    ...
    def _setup_step_sequencer2(self, as_active):
        if(self._stepseq2 != None):
            ...
            if as_active:
                ...
                # Update OSD once on enable to populate attributes without a per-frame cost.
                if not self._stepseq2_active:
                    self._stepseq2_active = True
                    self._stepseq2._update_OSD()
            else:
