...
class Launchpad(ControlSurface):
    ...
    def __init__(self, c_instance):
        ...
        self._mk2_rgb = False
        self._mk3_rgb = False
        self._led_cache = ['' for _ in range(80)]
        ...
        with self.component_guard():
            ...
            for row in range(8):
                ...
                for column in range(8):
                    ...
                    button._osd_led_id = ("g", row, column)
            ...
            for index, button in enumerate(top_buttons):
                button._osd_led_id = ("t", index)
            for index, button in enumerate(side_buttons):
                button._osd_led_id = ("s", index)
            ...
            self._osd.name = "OSD"
            # Cache LED values for the OSD (grid + top + side buttons).
            self._led_cache = ['' for _ in range(80)]
            self._osd.set_grid_leds_provider(self._get_led_cache_snapshot)
            if self._mk2_rgb or self._mk3_rgb or self._lpx:
    ...
    def _get_led_cache_snapshot(self):
        return list(self._led_cache)

    def _cache_led_value(self, control, value):
        led_id = getattr(control, "_osd_led_id", None)
        if led_id is None:
            return
        index = None
        if led_id[0] == "t":
            ...
            index = 8 + (led_id[1] * 8) + led_id[2]
        elif led_id[0] == "s":
            index = 72 + led_id[1]
        if index is None or index < 0 or index >= len(self._led_cache):
            return
        self._led_cache[index] = value
